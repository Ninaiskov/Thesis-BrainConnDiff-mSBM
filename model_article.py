...
############################################################### Gibbs sampler ###############################################################
    def gibbs_sample_Z(self, Z, JJ, comp, Force):
        ...
        n_link = self.compute_n_link(Z=Z, noc=self.noc, add_eta0=True, eta0=self.eta0)
        ...
        for i in JJ: # for each node (in random permutated order)
            ...
            #ZAi = np.stack([Z @ self.A[:, i, s] for s in range(self.S)], axis=1) # # sum of links between node i and other nodes in respective cluster/block for subject s
            ZAi = np.stack([(Z @ As[:,i]).squeeze() for As in self.A], axis=1)  # NEW
            ZAi = ZAi[:, np.newaxis, :]
...
############################################################### Data processing functions ###############################################################
    ...
    def load_articledata(self):
        ...
        '''
        # code for loading each scipy sparse csr graphs and append to list
        self.A = []
        for file in range(len(filenames)): # 5 fmri and 5 dmri
           graph = load_npz(os.path.join(data_path,filename)) # single graph
           self.A.append(graph)
        '''
...
############################################################### Model evaluation functions ###############################################################
    ...
    def compute_n_link(self, Z, noc, add_eta0, eta0):
        ...
        if add_eta0:
            ...
            n_link = n_link - 0.5 * np.eye(noc)*n_link
        return n_link
